    ...
    if [ $? -eq 0 ]; then
        ...
        if [ "$RegName" == "AppleBacklightDisplay" -o "$DisplayFlags" == "$(echo AAgAAA== | base64 -D)" ] ||
           [ "$RegName" == "AppleDisplay" -o "$DisplayFlags" == "$(echo AAgAAA== | base64 -D)" ]; then
            ...
            /usr/libexec/PlistBuddy -c "Add :EDID dict" "$GenPlist"
#            /usr/libexec/PlistBuddy -c "Add :EDID:DisplayProductName string 'Display'" "$GenPlist"
            /usr/libexec/PlistBuddy -c "Add :EDID:ProductID string 0x$ProdIDhex" "$GenPlist"
            /usr/libexec/PlistBuddy -c "Add :EDID:VendorID string 0x$VenIDhex" "$GenPlist"
            /usr/libexec/PlistBuddy -c "Add :EDID:Inject bool true" "$GenPlist"
...
#            /usr/libexec/PlistBuddy -c "Remove :test" "$GenPlist"
            /usr/libexec/PlistBuddy -c "Import :EDID:Custom /tmp/EDID.bin" "$GenPlist"
